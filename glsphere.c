...
void sphere(int maxlevel,int isolid) {
  ...
  for(s = 0; s < 20; s++) {
    ...
    for(i = 0; i < nrows; i++) {
      ...
      if(isolid) glBegin(GL_TRIANGLE_STRIP); 
...
float *sphere_tris(int maxlevel) {
  ...
  for(s = 0; s < 20; s++) {
    ...
    for(i = 0; i < nrows; i++) {
      ...
      lerp(&t->pt[1], &t->pt[2], (float)(i+1)/nrows, &v2);
      lerp(&t->pt[1], &t->pt[2], (float)i/nrows, &v3);
#define V(a, c, v) { point x = v; normalize(&a); normalize(&c); normalize(&x); b[0] = a.x; b[1] = a.y; b[2] = a.z; b[3] = c.x; b[4] = c.y; b[5] = c.z; b[6] = x.x; b[7] = x.y; b[8] = x.z; b+=9; }
      x1 = v0;
      x2 = v1;
      for(j = 0; j < i; j++) {
	/* calculate 2 more vertices at a time */
